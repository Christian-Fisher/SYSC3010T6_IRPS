...
public class ServerUDP {
	...
	private final static String OCCUPANCYUPDATECOMMAND = "OCC";
	...
	public ServerUDP() {
		try {
			...
			IncomingSocket.setSoTimeout(7000); // Sets the timeout time to 2 seconds so the incoming socket will throw
	...
	public void sendToLED(String spot, Boolean Occupancy) {
		String data = LEDCOMMAND + COMMANDSPLITREGEX; // Prefix the data being sent with "LED:" to indicate the command
		...
		try {
			DatagramPacket ack = new DatagramPacket(new byte[PACKETSIZE], PACKETSIZE); // create a packet to receive the
			...
			IncomingSocket.receive(ack); // Wait for a response from the Parking Controller. If the receive timesout, it
	...
	public void sendToArduino(Boolean PinCorrect) {
		String data = "Arduino" + COMMANDSPLITREGEX; // Prefix the message with "Arduino:" to signal the message is
		...
		try {
			DatagramPacket ack = new DatagramPacket(new byte[PACKETSIZE], PACKETSIZE); // create a packet which will use
			...
			OutgoingSocket.receive(ack); // Receive the acknowldgement, if this timesout, the exception will be caught,
	...
	public String heartbeatParking() {
		DatagramPacket heartBeat = new DatagramPacket(HEARTBEATMESSAGE, HEARTBEATMESSAGE.length,
		...
		while (run) {
			try {
				...
				if (!heartbeatParkingResponse.equals(NOTHINGTOREPORT)) {
					...
					String[] split1String = message.split(COMMANDSPLITREGEX);

...
void setup() {
  ...
  servoEntry.attach(11);      // attaching the servo motor function to pin 9
  servoEntry.write(0);       // intial set up of the position of the servo motor which is now at 0 degrees
  servoExit.attach(12);      // attaching the servo motor function to pin 9
  servoExit.write(0);
...
void exitGate(){
  ...
  //val = digitalRead(IRsensor);     // read the input pin
  //Serial.print(val);
  ...
  if(digitalRead(IRsensor)){
    ...
    delay(1000);
      ...
      printf("no car near the exit gate");
    }
...
void entryGate(boolean pinOK) {
  if (pinOK==false){
    ...
    delay(5000);
    for (rotate = 100; rotate >= 1; rotate -= 1) { // close gate after a delay
    servoEntry.write(rotate);
    delay(15);
    }
    delay(5000);
      ...
      printf("invalid pin / gate remains closed");
    }
